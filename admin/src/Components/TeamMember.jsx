...
const TeamMember = ({open, setOpen, teamData = null}) => {
    ...
    const handleCreateTeamMember = async () => {
        try {
            const response = await axios.post('http://localhost:9000/v1/teams/', {
    ...
    const updateTeamMember = async () => {
        try {
            const response = await axios.patch(`http://localhost:9000/v1/teams/team/${teamData.id}`, {
                name, image, designation,
            }, {
                withCredentials: true, headers: {
                    Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))?.access.token}`,
    ...
    const deleteTeamMember = async (e) => {
        e.preventDefault();
        try {
            const response = await axios.delete(`http://localhost:9000/v1/teams/team/${teamData.id}`, {
                withCredentials: true, headers: {
                    Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))?.access.token}`,
                },
            });
    ...
    const uploadSingleImage = async (e) => {
        ...
        try {
            const response = await axios.post('http://localhost:9000/v1/teams/upload/image', formData, {
                withCredentials: true, headers: {
                    Authorization: `Bearer ${JSON.parse(localStorage.getItem('token'))?.access.token}`,
                },
            });
